...
> **Swagger UI** is auto-generated by FastAPI at `http://localhost:{port}/docs` when `DEBUG=true`.
...
### GET `/health`
Health check for load balancers and Kubernetes probes. Returns `{"status": "healthy"}`.
...
All submission endpoints require `Authorization: Bearer <access_token>`.
...
### GET `/api/v1/status`
Service status with version and uptime info.
