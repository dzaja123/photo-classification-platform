...
class Submission(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Submitter's name"
    )
    age: Mapped[int] = mapped_column(Integer, nullable=False, comment="Submitter's age")
    gender: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Submitter's gender"
    )
    location: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Submitter's location (city)"
    )
    country: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Submitter's country"
    )
    ...
    photo_filename: Mapped[str] = mapped_column(
        String(500), nullable=False, comment="Original filename"
    )
    photo_path: Mapped[str] = mapped_column(
        String(1000), nullable=False, unique=True, comment="Path in MinIO storage"
    )
    ...
    classification_error: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Error message if classification failed"
    )
